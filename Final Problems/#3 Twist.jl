...
nt = 25 # number of chord distribution values
x0 = ones(nt) #[1.0, 0.86, 0] # starting point
lx = 0.0*ones(nt) # lower bounds on x
ux = (10*pi/180)*ones(nt)  # upper bounds on x
ng = 1+(nt-1) # number of constraints
...
println("xstar in radians = ", xopt)
xopt = xopt.*(180/pi)
println("xstar in degrees = ", xopt)
println("fstar = ", fopt)
println("info = ", info)